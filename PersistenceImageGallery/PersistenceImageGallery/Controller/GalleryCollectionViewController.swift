...
class GalleryCollectionViewController: UICollectionViewController, UICollectionViewDragDelegate, UICollectionViewDropDelegate, UICollectionViewDelegateFlowLayout
{
    ...
    {
        get {
            return ImageGallery(imagesURL: galleryURLs, scale: Float(scalingForCells))
        }
        set {
            guard let scale = newValue?.scale else { return }
            guard let imagesURL = newValue?.imagesURL else { return }
            scalingForCells = CGFloat(scale)
            galleryURLs = imagesURL
        }
    }
    ...
    @IBAction func saveDocuments(_ sender: UIBarButtonItem)
    {
    ...
    @IBAction func doneWithDocuments(_ sender: UIBarButtonItem)
    {
        if document?.imageGallery != nil {
